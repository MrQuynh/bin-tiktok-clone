...
function Home() {
    ...
    const [page, setPage] = useState(localStorage.getItem('HOME_PAGE') || 1);
            ...
            .then((res) => {
                if (res) {
                    ...
                    if (res.meta?.pagination) {
                        const pageNext = res?.meta?.pagination?.total_pages;
                        localStorage.setItem('HOME_PAGE', JSON.stringify(Math.floor(Math.random() * pageNext)));
                    }
                }
        ...
        (node) => {
            ...
            observer.current = new IntersectionObserver((entries) => {
                if (entries[0].isIntersecting) {
                    console.log('hello');
                    let pageNew = localStorage.getItem('HOME_PAGE') || page + 1;
                    setPage(pageNew);
                }
            });
