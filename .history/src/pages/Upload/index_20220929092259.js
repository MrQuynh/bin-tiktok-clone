...
function Upload() {
    ...
    const handleValueVideo = async (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = (e) => {
                ...
                // setValueVideo(e.target.result);
            };
            setHasVideo(true);
        }
    };
    const formData = new FormData(videoUrl);
    formData.append('upload_file', videoUrl);
    // const dataSend = {
    //     formData,
    //     upload_file: videoUrl,
    //     // description: 'hi',
    //     thumbnail_time: 4,
    //     viewable: 'public',
    //     allows: ['comment'],
    ...
    // formData.append('viewable', 'akjsd');
    // formData.append('allows', ['videoUrl']);
    console.log(formData);
    const handleSubmit = () => {
        // console.log(formData);
        userService.postCreateVideo(formData).then((data) => console.log(data));
    };

    return (
        <div className={cx('wrapper')}>
            <form method="POST" enctype="multipart/form-data" id="form-upload" className={cx('body')}>
