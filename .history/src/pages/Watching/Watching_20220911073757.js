...
const initListCmt = JSON.parse(localStorage.getItem('USER_LOG_IN')).innitListCmt;
function Watching() {
    ...
    // const [listCmts, setListCmts] = useState(state.commentsNow);
    ...
    useEffect(() => {
        linkVideo &&
            userService.getListComments(linkVideo.uuid).then(
                (data) => dispatch(actions.setCommentsNow(...data)),
                // setListCmts(data)
            );
    }, [linkVideo]);
    ...
    const dataSend = { comment: value };

    const handleSubmit = () => {
        linkVideo && userService.postCreateNewComment(linkVideo.uuid, dataSend);
        dispatch(actions.setCommentsNow(value));
        setValue('');
        inputRef.current.focus();
        // lastRef.current.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
        // console.log(linkVideo.uuid);
        // window.location.reload();
    };
    const lastRef = useRef();
    console.log(state.commentsNow);
    return (
        <div className={cx('wrapper')}>
            ...
            <div className={cx('author')}>
                ...
                <div className={cx('comments')}>
                    {state.commentsNow &&
                        state.commentsNow.reverse().map((data, index) => {
                            if (index === state.commentsNow.length) {
                                console.log(index === state.commentsNow.length);
                                return <UserComment itemCmts={data} ref={lastRef} key={index} />;
                            } else {
                                return <UserComment itemCmts={data} key={index} />;
                            }
                        })}
                </div>
