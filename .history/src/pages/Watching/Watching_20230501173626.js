...
function Watching() {
    ...
    const [idVideo, setIdVideo] = useState(query.get('id'));
    // setIdVideo();
    ...
    const handleChangeVideo = () => {
        const listVideoRedux = state.listVideoWatchingRedux;
        if (listVideoRedux.length > 1) {
            let inVideo = listVideoRedux.findIndex((item) => item.id == idVideo) || 0;
            if (inVideo === listVideoRedux.length - 1) {
                inVideo = 0;
            } else {
                inVideo++;
            }

            setIdVideo(listVideoRedux[inVideo].id);
            setIndexVideo(inVideo);
            // console.log('id', listVideoRedux[inVideo].id);
        }
    };
    // change back video
    const handleChangeBackVideo = () => {
        const listVideoRedux = state.listVideoWatchingRedux;
        if (listVideoRedux.length > 1) {
            let inVideo = listVideoRedux.findIndex((item) => item.id == idVideo) || 0;
            if (inVideo === 0) {
                inVideo = listVideoRedux.length - 1;
            } else {
                inVideo--;
            }
        ...
        if (e.key === 'Enter') {
            if (value) {
                ...
                setListCmts((prev) => [
                    {
                        comment: value,
                        // created_at: moment(new Date().toISOString()).startOf('day').fromNow(),
                        user: {
                            nickname: user.nickname,
                            avatar: user.avatar,
                        },
    ...
    return (
        <div className={cx('wrapper')}>
            ...
            {loading ? (
                ...
                <div className={cx('body-video')} onClick={() => setCount(count + 1)} onKeyDown={handleKeyDownVideo}>
                    <div>
                        {linkVideo && (
                    ...
                    {/* change video */}
                    <div className={cx('icon-body', 'up', indexVideo === 0 && 'hidden')}>
                        <BsChevronUp className={cx('icon')} onClick={handleChangeBackVideo} />
                    </div>
                    <div className={cx('icon-body', 'down', state.listVideoWatchingRedux === 1 && 'hidden')}>
